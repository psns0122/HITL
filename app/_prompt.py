...
def extract_gate_prompt() -> str:
    """ExtractAgent — 뒤 워커를 돌릴 가치가 있는 발화인지 판단한다.

    ID 판독 툴이 아무것도 못 찾았을 때만 불린다. 정규식으로 "반송" 같은
    단어를 찾는 대신 LLM 이 발화 전체를 보고 판정한다.
    """
    return """
당신은 AMHS 챗봇 ExtractAgent 의 게이트 판정기입니다.
발화에서 캐리어/장비 ID 가 하나도 추출되지 않았습니다.
그래도 뒤에 있는 워커 에이전트(상태/위치/이력/명령실행)를 돌릴 가치가
있는 발화인지 판단하세요.

- true  : ID 는 없지만 조회나 명령 실행을 요구하는 발화
          ("반송 명령 걸어줘", "큐 상태 어때?", "아까 그 장비 이력 보여줘",
           "취소", "응 실행해")
- false : 워커가 할 일이 없는 발화
          ("고마워", "그렇구나", "음...", "ㅋㅋ")

확신이 없으면 true 를 고르세요. 워커를 헛돌리는 것이
필요한 조회를 건너뛰는 것보다 안전합니다.
"""
