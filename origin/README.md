...
| 태그 | 뜻 | 해당 파일 |
|---|---|---|
| `[원본·직접]` | 사용자가 직접 타이핑/지시해 준 코드. 구조·변수명 그대로. | `_agent.py`, `_node.py` 의 `extract_node` / `supervisor_node`, `_tool.py` 의 시그니처, `api/graph_service.py` |
...
| `[원본·추정]` | 위 셋 어디에도 원문이 없어 같은 패턴으로 채운 부분. **사내 실물과 대조 필요.** | `_prompt.py`, `api/schemas.py`, `main.py`, `_util.safe_tool`, `_tool.py` 의 ActionAgent 툴 2종, `config.VALID_FABS` |
...
| 체크포인터 | `MemorySaver` — 프로세스 공용 하나 | 동일 (`InMemorySaver`, langgraph 1.x 이름) |
| 모델 선택 | `state["model_name"]` 으로 노드가 읽음 | 동일 + `/models` API (프론트 드롭다운용) |
